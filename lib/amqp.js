...
export function consumeTo (qName, consumeFunc, consumeOptions = { noAck: true }) {
  return getCh()
    .then(ch =>  ch.consume(qName, consumeFunc, consumeOptions));
}

export function checkBindAndConsumeTo ({ exProps, qProps, keys, consumeFunc, consumeOptions = { noAck: true } }) {
  const props = {
    exProps: exProps,
    qProps: qProps,
    keys,
...
export function checkBindExchanges () {
  return ch.assertExchange('main', 'topic', { durable: true })
    .then(() => ch.assertExchange('sysms', 'direct', { durable: true }))
    .then(() => ch.assertExchange('regs', 'direct', { durable: true }))
    .then(() => ch.assertQueue('log', { durable: true }))
    .then(() => Promise.all([
      ch.bindQueue('log', 'main', '#'),
      ch.bindExchange('sysms', 'main', 'sysms.#'),
      ch.bindExchange('regs', 'main', 'regs.#'),
    ]));
}
